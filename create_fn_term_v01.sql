...
CREATE TABLE `fn_term` (
  ...
  #`NON_INTEREST_BEARING_UPB` decimal(12,2) unsigned DEFAULT NULL,/* mod related */ /* 63 */
  #`PRINCIPAL_FORGIVENESS_AMOUNT` decimal(12,2) unsigned DEFAULT NULL,/* mode related */ /* 64 */
  ...
  `RE_PROCS_FLAG` enum('Y','N') DEFAULT NULL,/* repurchase, always w/ zero_bal_code */ /* 105 */
  ...
  constraint fn_term_act_period_ck
	     ...
	     and cast(substring(act_period from 5 for 2) as unsigned)
	       between 1 and 12))),
  constraint fn_term_zb_dte_ck
  	     check((zb_dte is null or
	     (cast(substring(zb_dte from 1 for 4) as unsigned)
               between 1990 and 2999
	     and cast(substring(zb_dte from 5 for 2) as unsigned)
	       between 1 and 12))),
  constraint fn_term_foreclosure_date_ck
  	     check((foreclosure_date is null or
	     (cast(substring(foreclosure_date from 1 for 4) as unsigned)
               between 1990 and 2999
	     and cast(substring(foreclosure_date from 5 for 2) as unsigned)
	       between 1 and 12))),
  constraint fn_term_disposition_date_ck
  	     check((disposition_date is null or
	     (cast(substring(disposition_date from 1 for 4) as unsigned)
               between 1990 and 2999
	     and cast(substring(disposition_date from 5 for 2) as unsigned)
	       between 1 and 12))),
  constraint fn_term_qfile_ck
  ...
  KEY `zero_bal_code_idx` (`ZERO_BAL_CODE`),
  KEY `qfile_idx` (`QFILE`)
)
